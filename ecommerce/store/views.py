...
def cart_detail(request, total=0, counter=0, cart_items=None):
    ...
    description = 'AlhadBhadekar - New Order'
...
# http://127.0.0.1:8000/accounts/login/?next=/order_history/
# IN default we see next, so adding next in redirect_field_name
...
def viewOrder(request, order_id):
    if request.user.is_authenticated:
        ...
        order_items = OrderItem.objects.filter(order=order)  # This will be a list as 1 Order has multiple order items
